...
- **Client-Side Image Processing:** Image manipulation (cropping/scaling) is performed in the browser using `react-easy-crop` to reduce server load and latency.
- **Client-Side Video Compression:** Video compression is handled via `FFmpeg WASM` (`@ffmpeg/ffmpeg`) running entirely in the browser, ensuring optimized file sizes before upload.
- **Cloud Storage:** All optimized assets are piped to Supabase Storage, utilizing signed URLs for secure access.
...
- **Tailwind Integration:** The design system is centralized in `tailwind.config.cjs`, utilizing custom tokens for colors, shadows, and border-radius to ensure design consistency across the application.
...
- **Performance:** Offloading media processing to the browser (FFmpeg WASM, react-easy-crop) minimizes bandwidth and server costs.
